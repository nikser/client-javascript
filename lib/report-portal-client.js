   ...
   */
    ...
    startLaunch(launchDataRQ) {
        ...
        if (launchDataRQ.id) {
            ...
            this.launch_id = launchDataRQ.id;
        } else {
    ...
    mergeLaunches() {
        if (this.isLaunchMergeRequired) {
            const launch_ids = helpers.readLaunchesFromFile();
            const params = new URLSearchParams({
                'filter.in.uuid': launch_ids,
            });
        ...
        Promise.all(itemObj.childrens.map(itemId => this.map[itemId].promiseFinish))
            .then(() => {
                ...
                this.finishTestItemPromiseStart(itemObj,
                    itemTempId,
                    Object.assign(finishTestItemData, { launch_id: this.launch_id }));
            }, () => {
                this.cleanMap(itemObj.childrens);
    ...
    saveLog(itemObj, requestPromiseFunc) {
        ...
        this.map[tempId] = this.getNewItemObj((resolve, reject) => {
            itemObj.promiseStart.then(() => {
                ...
                requestPromiseFunc(itemObj.realId, this.launch_id).then((response) => {
    ...
    sendLogWithoutFile(itemTempId, saveLogRQ) {
        ...
        const requestPromise = (itemUuid, launch_id) => {
            const url = 'log';
            const isItemUuid = itemUuid !== launch_id;
            // eslint-disable-next-line max-len
            return this.restClient.create(url, Object.assign(saveLogRQ, { launch_id }, isItemUuid && { itemUuid }), { headers: this.headers });
        };
    ...
    sendLogWithFile(itemTempId, saveLogRQ, fileObj) {
        ...
        const requestPromise = (itemUuid, launch_id) => {
            ...
            // eslint-disable-next-line max-len
            return this.getRequestLogWithFile(Object.assign(saveLogRQ, { launch_id }, isItemUuid && { itemUuid }), fileObj);
        };
    ...
    finishTestItemPromiseStart(itemObj, itemTempId, finishTestItemData) {
        itemObj.promiseStart
            .then(() => {
                ...
                this.logDebug(`Finish test item ${itemTempId}`);
                // eslint-disable-next-line max-len
                this.restClient.update(url, Object.assign(finishTestItemData, { launch_id: this.launch_id }), { headers: this.headers })
                    .then((response) => {
